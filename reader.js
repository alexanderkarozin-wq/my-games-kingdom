...
const IMAP_POINT_REFERENCE_WIDTH = 760;
...
const readerImagePreloadCache = new Map();
...
function renderStoryText(scene) {
  ...
  const reveal = shouldRevealText(scene);
  if (isRichTextHtml(raw)) return renderRichStoryHtml(raw, scene, reveal);
...
function isRichTextHtml(value) {
  return /<\/?(a|p|h[1-3]|ul|ol|li|strong|em|span|blockquote|br|div|figure|img|hr|u|s|strike)\b/i.test(String(value || ""));
}

function renderRichStoryHtml(raw, scene, reveal = false) {
  const template = document.createElement("template");
  template.innerHTML = sanitizeRichTextHtml(raw || "");
  let secretUsed = false;
  const markerNodes = [];
  const walker = document.createTreeWalker(template.content, NodeFilter.SHOW_TEXT);
  while (walker.nextNode()) markerNodes.push(walker.currentNode);
  markerNodes.forEach((textNode) => {
    if (!textNode.nodeValue.includes(SECRET_MARKER)) return;
    if (!scene.hidden?.enabled || secretUsed) {
      textNode.nodeValue = textNode.nodeValue.split(SECRET_MARKER).join("");
      return;
    }
    const [before, ...rest] = textNode.nodeValue.split(SECRET_MARKER);
    const after = rest.join("");
    const fragment = document.createDocumentFragment();
    if (before) fragment.append(document.createTextNode(before));
    const buttonTemplate = document.createElement("template");
    buttonTemplate.innerHTML = secretButtonHtml(scene);
    fragment.append(buttonTemplate.content.cloneNode(true));
    if (after) fragment.append(document.createTextNode(after));
    textNode.replaceWith(fragment);
    secretUsed = true;
  });
  template.content.querySelectorAll("h1, h2, h3").forEach((node) => node.classList.add("story-heading"));
  template.content.querySelectorAll("blockquote").forEach((node) => node.classList.add("story-quote"));
  if (reveal) {
    template.content.querySelectorAll("p, h1, h2, h3, blockquote, li").forEach((node) => {
      if (node.closest(".qe-callout")) return;
      node.classList.add("reveal-block", "reveal-unit");
    });
    template.content.querySelectorAll(".qe-callout").forEach((node) => {
      node.classList.add("reveal-block", "reveal-unit");
    });
  }
  ...
  if (scene.hidden?.enabled && !secretUsed) {
    const button = secretButtonHtml(scene);
    html += reveal ? `<p class="reveal-block reveal-unit">${button}</p>` : `<p>${button}</p>`;
  }
  return html || "<p></p>";
}
...
function startQte(scene) {
  ...
  const finish = (ok) => {
    ...
    if (ok) {
      ...
      playSound("failSound");
      showInlineResult(status, systemText(scene.qte?.failureText, "Провал", "qteFailure"), scene.qte?.failure, "bad");
    }
  };
  const updateStatus = () => {
...
function collectInteractiveMapImageSources(map = interactiveMap) {
  const sources = [
    map?.mapImage,
    map?.coverImage,
    map?.theme?.backgroundImage,
    map?.theme?.mapBackdropImage,
  ];
  (map?.points || []).forEach((point) => {
    sources.push(point.pointImage, point.contentImage, point.puzzleImage);
  });
  return Array.from(new Set(sources.map((source) => String(source || "").trim()).filter(Boolean)));
}

function collectInteractiveMapDisplayImageSources(map = interactiveMap) {
  const sources = [
    map?.mapImage,
    map?.theme?.mapBackdropImage,
  ];
  (map?.points || []).forEach((point) => {
    if (point.kind === "image") sources.push(point.pointImage);
  });
  return Array.from(new Set(sources.map((source) => String(source || "").trim()).filter(Boolean)));
}

function preloadReaderImage(source) {
  const src = String(source || "").trim();
  if (!src) return null;
  const cached = readerImagePreloadCache.get(src);
  ...
  readerImagePreloadCache.set(src, record);
  return record;
}

function isReaderImageSettled(source) {
  const src = String(source || "").trim();
  if (!src) return true;
  const record = readerImagePreloadCache.get(src);
  return Boolean(record && record.status !== "loading");
}

function areReaderImagesSettled(sources = []) {
  return sources.every(isReaderImageSettled);
}

function preloadInteractiveMapAssets(map = interactiveMap) {
  if (!map) return;
  const records = collectInteractiveMapImageSources(map).map(preloadReaderImage).filter(Boolean);
  if (!records.length) return;
  Promise.allSettled(records.map((record) => record.promise)).then(() => {
    if (contentKind !== "map" || interactiveMap !== map) return;
    const stage = document.getElementById("readerStage");
    if (stage?.querySelector("[data-reader-map-loading]")) renderMapReader();
  });
}
...
function renderMapReader() {
  ...
  preloadInteractiveMapAssets(interactiveMap);
  ...
  const mapDisplayReady = areReaderImagesSettled(collectInteractiveMapDisplayImageSources(interactiveMap));
  ...
  stage.innerHTML = `
    <section class="imap-reader-stage reader-map-stage${isFullscreen ? " is-map-fullscreen" : ""}">
      ...
      <div class="imap-reader-board${mapDisplayReady ? "" : " is-loading"}" data-reader-map-board style="--imap-zoom:${zoom};--imap-pan-x:${panX}px;--imap-pan-y:${panY}px">
        ...
        ${mapDisplayReady ? "" : `
          <div class="imap-map-loading" data-reader-map-loading>
            <span class="image-spinner"></span>
            <span>${t("loading")}</span>
          </div>
        `}
...
function updateReaderMapViewportScale(stage = document.getElementById("readerStage")) {
  ...
  const width = layer?.offsetWidth || layer?.clientWidth || board.getBoundingClientRect?.().width || board.clientWidth || IMAP_POINT_REFERENCE_WIDTH;
  const scale = clampNumber(width / IMAP_POINT_REFERENCE_WIDTH, 0.55, 2.8);
...
function mapActivityControls(point, completed) {
  ...
  if (completed && point.activity !== "popup") {
    return `<div class="answer-status is-good"><span>${t("pointCompleted")}</span></div><button class="answer-button" type="button" data-reader-map-close><span>${t("close")}</span></button>`;
  }
  ...
  if (point.activity === "popup") {
    return `<button class="answer-button" type="button" data-reader-map-close data-reader-map-read-complete><span>${buttonLabel}</span></button>`;
  }
...
function showReaderMapModal(innerHtml) {
  ...
  modal.querySelectorAll("[data-reader-map-close]").forEach((button) => button.addEventListener("click", () => {
    ...
    const shouldCompletePopup = button.hasAttribute("data-reader-map-read-complete");
    if (shouldCompletePopup && point?.activity === "popup" && !mapPlayState.completed.includes(point.id)) {
      completeReaderMapPoint(point, true);
      return;
    }
    modal.remove();
  }));
...
function formatMapRichText(value) {
  if (isRichTextHtml(value)) return `<div class="imap-rich-text">${sanitizeRichTextHtml(value)}</div>`;
...
function enhanceClientImages(root) {
  ...
  root?.querySelectorAll?.(CLIENT_ZOOM_IMAGE_SELECTOR).forEach((image) => {
    ...
    if (
      image.classList.contains("story-image")
      || image.classList.contains("imap-activity-image")
      || image.classList.contains("imap-puzzle-reference")
      || image.closest(".search-image")
      || image.closest(".digest-content")
    ) {
      frame.classList.add("is-block");
    }
...
function renderFormattedTextBlock(value, className = "") {
  ...
  if (isRichTextHtml(raw)) {
    ...
    template.content.querySelectorAll("blockquote").forEach((node) => node.classList.add("story-quote"));
    if (className) {
      template.content.querySelectorAll("p, h1, h2, h3, blockquote, ul, ol, .qe-callout").forEach((node) => node.classList.add(className));
    }
    return template.innerHTML;
  }
...
function normalizeQuest(data) {
  ...
  base.scenes = base.scenes.map((raw, index) => ({
    ...
    qte: {
      ...
      failureText: String(raw?.qte?.failureText || "Провал"),
  ...
  if (!base.scenes.length) {
    base.scenes.push({
      ...
      qte: { buttonLabel: "Начать испытание", image: "", speed: 1400, count: 4, allowedMisses: 1, successText: "Испытание пройдено", failureText: "Провал", success: "", failure: "" },
